...
def conv2d(x, output, kernel_size, strides, padding=0, name="conv2d"):
    with tf.variable_scope(name):
        ...
        x = tf.nn.conv2d(x, w, strides=strides, padding='VALID') + b
        return tf.pad(x, [[0, 0], [padding, padding], [padding, padding], [0, 0]])
...
def deconv2d(x, output_shape, kernel_size, strides, padding=0, name="deconv2d"):
    with tf.variable_scope(name):
        ...
        x = tf.nn.conv2d_transpose(x, w, output_shape=output_shape, strides=strides, padding='VALID') + b
        return tf.pad(x, [[0, 0], [padding, padding], [padding, padding], [0, 0]])
